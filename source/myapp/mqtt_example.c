...
__construct:
...
#ifdef AIR720
        struct at_device *netdevice = at_device_get_by_name(AT_DEVICE_NAMETYPE_NETDEV,"air720");
#endif
#ifdef EC200X
        struct at_device *netdevice = at_device_get_by_name(AT_DEVICE_NAMETYPE_NETDEV,"ec200x");
#endif
    ...
    while (1)
    {
        if (data_ready == 1 || datereport == 1 || state_changed == 1)
        {
            ...
            if (state_changed == 0)
            {
                if (time_curr - time_prev > remainms)
                {
                    refresh_data = 1;
                    while (refresh_data == 1)
                    {
                    ...
                    if (test_publish(pclient) == 0)
                    {
                        data_ready = 0;
                        datereport = 0;
                        waitms = waitms > (1200000 / 5) ? 1200000 : waitms * 5;
                        remainms = waitms;
                    }
                }
                else
                {
                    remainms -= time_curr - time_prev;
                }
            }
            else
            {
