...
#define MQTT_EVENT_CANPUB (1L << 6)
#define MQTT_EVENT_CONNECTED (1L << 7)
#define MQTT_EVENT_DISCONNECTED (1L << 8)
...
//	t2 = message;
	...
	mqtt_event_recv(mqtt_event, MQTT_EVENT_PUBACK | MQTT_EVENT_CANPUB, 0, RT_EVENT_FLAG_OR);
	//TODO qos 1
	char string[1000];
	sprintf(string, "AT+QMTPUBEX=0,0,%d,0,\"%s\",%d\r\n", qos, topic, msg_len);
	at_client_send(string, strlen(string));
	int event_result = mqtt_event_recv(mqtt_event, MQTT_EVENT_CANPUB, rt_tick_from_millisecond(request_timeout_ms), RT_EVENT_FLAG_OR);
	...
	at_client_send(message, msg_len);
	// at_resp_set_info(resp, 128, 0, rt_tick_from_millisecond(request_timeout_ms));
	// if (at_exec_cmd(resp, "%s%c",message,0x1a) < 0)
	// {
	// 	result = -RT_ERROR;
	// 	goto __exit;
	// }
	event_result = mqtt_event_recv(mqtt_event, MQTT_EVENT_PUBACK, rt_tick_from_millisecond(request_timeout_ms), RT_EVENT_FLAG_OR);
	if (event_result == -RT_ETIMEOUT)
	{
		LOG_E("time out");
		result = -RT_ERROR;
...
int HAL_AT_MQTT_State(void)
{
	...
	if (checktime > 2000 * 24 * 60 * 2)
	{
		...
		if (at_exec_cmd(resp, "AT+QLTS=2") == 0)
		{
			at_resp_parse_line_args_by_kw(resp, "+QLTS:", "+QLTS: \"%d/%d/%d,%d:%d:%d+%*d,%*d\"",parsed_data,parsed_data+1,parsed_data+2,parsed_data+3,parsed_data+4,parsed_data+5);
			rt_kprintf("get %d,%d,%d,%d,%d,%d",parsed_data[0],parsed_data[1],parsed_data[2],parsed_data[3],parsed_data[4],parsed_data[5]);
			set_date(parsed_data[0],parsed_data[1],parsed_data[2]);
	...
	if (statusc == 2000 || needcheck == 1)
	{
		if (mqtt_state == IOTX_MC_STATE_CONNECTED || needcheck == 1)
		{
			...
			mqtt_event_recv(mqtt_event, MQTT_EVENT_CONNECTED | MQTT_EVENT_DISCONNECTED, 0, RT_EVENT_FLAG_OR);
			at_client_send("AT+QMTCONN?\r\n", 14);
			int event_result = mqtt_event_recv(mqtt_event, MQTT_EVENT_CONNECTED | MQTT_EVENT_DISCONNECTED, rt_tick_from_millisecond(5000), RT_EVENT_FLAG_OR);
			if (event_result == -RT_ETIMEOUT)
			{
				urc_close_func(RT_NULL, RT_NULL, 0);
			}
			else if (event_result & MQTT_EVENT_DISCONNECTED)
			{
				urc_close_func(RT_NULL, RT_NULL, 0);
			}
			if (resp)
...
static void reconnect_entry(void *parameter)
{
	...
	at_response_t resp = RT_NULL;
	int parsed_data = -1;
	resp = at_create_resp(128, 4, 50 * RT_TICK_PER_SECOND);
	goto __reconnect;
...
__reconnect:
	...
	do
	{
		...
		{
			if ((HAL_AT_MQTT_Connect(_product_key, _device_name, _device_secret) == 0))
			{
				goto __exit;
			}
	...
	} while (HAL_AT_MQTT_Connect(_product_key, _device_name, _device_secret) != 0);
	goto __exit;
...
static void urc_connect_func(struct at_client *client, const char *data, rt_size_t size)
{
	...
	if (mqtt_state == IOTX_MC_STATE_INITIALIZED || mqtt_state == IOTX_MC_STATE_DISCONNECTED_RECONNECTING)
	{
		...
		if (strstr(data, "+QMTOPEN: 0,0") != 0)
		{
			mqtt_event_send(mqtt_event, MQTT_EVENT_CONN_OK);
		}
		else if (strstr(data, "+QMTCONN: 0,0"))
		{
	...
	{
		if (strstr(data, "+QMTCONN: 0,3"))
		{
			mqtt_event_send(mqtt_event, MQTT_EVENT_CONNECTED);
		}
		else
		{
			mqtt_event_send(mqtt_event, MQTT_EVENT_DISCONNECTED);
		}
	}
}
...
static void urc_send_func(struct at_client *client, const char *data, rt_size_t size)
{
	...
	if (strstr(data, ">"))
	{
		mqtt_event_send(mqtt_event, MQTT_EVENT_CANPUB);
		return;
	}
	if (strstr(data, "+QMTPUBEX: 0,0,0"))
	{
		mqtt_event_send(mqtt_event, MQTT_EVENT_PUBACK);
	}
	else if (strstr(data, "+QMTSUB: 0,1,0"))
	{
...
static void urc_recv_func(struct at_client *client, const char *data, rt_size_t size)
{
	if (mqtt_state == IOTX_MC_STATE_CONNECTED)
	{
		...
		int tt;
		sscanf(data, "+QMTRECV: 0,%*d,\"%[^\"]\",%d,\"%n", topic, (int *)&bfsz, &tt);
...
int HAL_AT_MQTT_Connect(char *proKey, char *devName, char *devSecret)
{
	...
	while (retry_n--)
	{
		if (at_exec_cmd(resp, "AT+QMTCFG=\"recv/mode\",0,0,1") < 0)
		{
			result = -RT_ERROR;
			goto __exit;
		}
		if (at_exec_cmd(resp, "AT+QMTCFG=\"session\",0,0") < 0)
		{
			result = -RT_ERROR;
			goto __exit;
		}
		...
		// 	continue;
		// }
		if (at_exec_cmd(resp, "AT+QMTCFG=\"aliauth\",0,\"%s\",\"%s\",\"%s\"", _product_key, _device_name, _device_secret) < 0)
		{
			result = -RT_ERROR;
			goto __exit;
		}
		...
		int mipresult = at_exec_cmd(resp, "AT+QMTOPEN=0,\"%s\",%d", host, port);
		...
		mipresult = at_exec_cmd(resp, "AT+QMTCONN=0,\"%d\"", HAL_Random(1000));
		if (mipresult == -RT_ERROR)
		{
			result = -RT_ERROR;
			goto __exit;
