...
#define PIN_MAX_NUM                     ((PortH * 16) + (__CLZ(__RBIT(Pin15))) + 1)
...
static void pin_irq_handler(rt_uint16_t pinbit)
{
    ...
    if (Set == EXINT_IrqFlgGet(pinbit))
    {
        EXINT_IrqFlgClr(pinbit);
...
static void hc32_pin_mode(rt_device_t dev, rt_base_t pin, rt_base_t mode)
{
    ...
    PORT_SetFunc(gpio_port, gpio_pin,Func_Gpio,Disable);
}
...
static void gpio_irq_config(uint8_t u8Port, uint16_t u16Pin, uint16_t u16ExInt)
{
    ...
    PCRx = (stc_port_pcr_field_t *)((uint32_t)(&M4_PORT->PCRA0) + u8Port * 0x40ul + pin_num * 0x04ul);
    PCRx->INTE = u16ExInt;
}
                                ...
                                
{
    ...
    if (enabled == PIN_IRQ_ENABLE)
    {
        ...
        stcExintInit.enExitCh    = gpio_pin;
        ...
        gpio_irq_config(GPIO_PORT(pin), gpio_pin, Enable);
    ...
    {
        level = rt_hw_interrupt_disable();
        gpio_irq_config(GPIO_PORT(pin), gpio_pin, Disable);
