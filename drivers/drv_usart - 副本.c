...
static void hc32_uart_rxto_irq_handler(struct hc32_uart *uart)
{
	...
	if (0UL != recv_total_index)
    {
        ...
        if (recv_len)
        {
//						rt_kprintf("to , recv_len: %d,last_index : %d\r\n",recv_len,uart->dma_rx_last_index);
...
int hc32_hw_uart_init(void)
{
    ...
    for (int i = 0; i < obj_num; i++)
    {
        ...
        /* register UART device */
			if(i==0){        result = rt_hw_serial_register(&uart_obj[i].serial,
                                       uart_obj[i].name,
                                       (RT_DEVICE_FLAG_RDWR   |
...
else{        result = rt_hw_serial_register(&uart_obj[i].serial,
                                       uart_obj[i].name,
                                       (RT_DEVICE_FLAG_RDWR   |
                                        RT_DEVICE_FLAG_INT_RX |
                                        RT_DEVICE_FLAG_INT_TX |
