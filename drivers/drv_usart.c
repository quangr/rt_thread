...
#define UART_RXTO_CONFIG(USART)                           \
    {                                                     \
        ...
        .timeout_bits = 100UL,                            \
        .irq_config = {.irq = USART##_RXTO_INT_IRQn,      \
...
#define UART_DMA_RX_CONFIG(USART)                           \
    {                                                       \
        ...
        .irq_config = {.irq = USART##_RX_DMA_INT_IRQn,      \
...
#define UART_DMA_TX_CONFIG(USART)                         \
    {                                                     \
        ...
        .irq_config = {.irq = USART##_TC_INT_IRQn,        \
...
#define DMA_LLP(unit, ch) \
    (READ_REG32(DMA_CH_REG((unit)->LLP0, (ch))))
...
static rt_err_t hc32_control(struct rt_serial_device *serial, int cmd, void *arg)
{
    ...
    case RT_DEVICE_CTRL_CONFIG:
        //			if(uart->Instance==M4_USART2){rt_kprintf("i'm here");}
...
static int hc32_putc(struct rt_serial_device *serial, char c)
{
    ...
    {
        if (USART_GetStatus(uart->Instance, UsartTxEmpty) != Set)
        {
            return -1;
        }
    }
...
static int hc32_getc(struct rt_serial_device *serial)
{
    ...
    uart = rt_container_of(serial, struct hc32_uart, serial);
    RT_ASSERT(RT_NULL != uart->Instance);
    //    while(Set != USART_GetStatus(uart->Instance, UsartRxNoEmpty)){};
...
static void hc32_uart_rxerr_irq_handler(struct hc32_uart *uart)
{
    ...
    {
        // rt_kprintf("UsartParityErr");
        USART_ClearStatus(uart->Instance,UsartParityErr);

    }
    if (Set == USART_GetStatus(uart->Instance, UsartFrameErr))
    {
        USART_ClearStatus(uart->Instance,UsartFrameErr);
        // rt_kprintf("UsartFrameErr");
    }
    if (Set == USART_GetStatus(uart->Instance, UsartOverrunErr))
    {
        // rt_kprintf("UsartOverrunErr");
        USART_ClearStatus(uart->Instance,UsartOverrunErr);
    }

    if (Set == USART_GetStatus(uart->Instance, (UsartParityErr))|Set == USART_GetStatus(uart->Instance,(UsartParityErr)))
    {
...
static void hc32_dma_config(struct rt_serial_device *serial, rt_ubase_t flag)
{
    ...
    if (RT_DEVICE_FLAG_DMA_RX == flag)
    {
...
#ifdef FULL
        dma_init.u16TransferCnt = serial->config.bufsz;
#else
        dma_init.u16TransferCnt = serial->config.bufsz / 2;
#endif
...
#else
        llp_desc.LLPx = (uint32_t)&llp_desc1;
#endif
        ...
        llp_desc1.SARx = dma_init.u32SrcAddr;
        llp_desc1.DARx = (uint32_t)(rx_fifo->buffer + serial->config.bufsz / 2);
        ;
        llp_desc1.DTCTLx = (((uint32_t)dma_init.u16TransferCnt) << DMA_DTCTL_CNT_Pos) | (dma_init.u16BlockSize << DMA_DTCTL_BLKSIZE_Pos);
        llp_desc1.LLPx = (uint32_t)&llp_desc;
        llp_desc1.CHxCTL = (dma_init.stcDmaChCfg.enSrcInc | ((uint32_t)dma_init.stcDmaChCfg.enDesInc) << (DMA_CHCTL_DINC_Pos) | ((uint32_t)dma_init.stcDmaChCfg.enTrnWidth) << (DMA_CHCTL_HSIZE_Pos) |
                            ((uint32_t)llp_init.u32LlpEn) << DMA_CHCTL_LLPEN_Pos | ((uint32_t)llp_init.u32LlpRun) << DMA_CHCTL_LLPRUN_Pos | ((uint32_t)llp_init.u32LlpEn) << DMA_CHCTL_IE_Pos);
...
static void hc32_uart_dma_rx_irq_handler(struct hc32_uart *uart)
{
...
#else
    recv_len = serial->config.bufsz / 2 - uart->dma_rx_last_index;
#endif
    uart->dma_rx_last_index = 0UL;
    rt_hw_interrupt_enable(level);
    //    ntaddr = DMA_LLP(uart->config.dma_rx->Instance , uart->config.dma_rx->channel);
    //	  if (ntaddr==uart->llp){
    //    recv_len = serial->config.bufsz/2 - uart->dma_rx_last_index;
    //    uart->dma_rx_last_index = 0UL;
    //			i=0;
    ...
    //		}
    //    rt_hw_interrupt_enable(level);
    //		if(i==1){		rt_kprintf("half , recv_len: %d,last_index : %d,first char:%c\r\n",recv_len,uart->dma_rx_last_index,*(((struct rt_serial_rx_fifo *)serial->serial_rx)->buffer));}
    //		if(i==0){		rt_kprintf("whole , recv_len: %d,last_index : %d ,first char:%c\r\n",recv_len,uart->dma_rx_last_index,*(((struct rt_serial_rx_fifo *)serial->serial_rx)->buffer));}
    if (recv_len)
    {
...
#else
    recv_total_index = uart->serial.config.bufsz / 2 - cnt;
#endif
...
static void hc32_uart1_rx_irq_handler(void)
{
    /* enter interrupt */
    rt_interrupt_enter();
    //	  rt_kprintf("uart1_rx_irq_handler");
    //	  rt_kprintf("now,%d",USART_GetStatus(uart_obj[UART1_INDEX].Instance, UsartRxNoEmpty));
