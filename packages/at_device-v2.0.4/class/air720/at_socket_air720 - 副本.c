...
__retry:
    ...
    if (air720_socket_event_recv(device, SET_EVENT(device_socket, 0), 100 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR) < 0)
    {
    ...
    event_result = air720_socket_event_recv(device,
                                            AIR720_EVENT_CONN_OK | AIR720_EVENT_CONN_FAIL, 10 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR);
    if (event_result < 0)
    {
...
static int air720_socket_send(struct at_socket *socket, const char *buff, size_t bfsz, enum at_socket_type type)
{
    ...
    while (sent_size < bfsz)
    {
        ...
        if (air720_socket_event_recv(device, SET_EVENT(device_socket, 0), 150 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR) < 0)
        {
        ...
        /* waiting OK or failed result */
        event_result = air720_socket_event_recv(device,
                                                AIR720_EVENT_SEND_OK | AIR720_EVENT_SEND_FAIL, 50 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR);
        if (event_result < 0)
        {
...
#define RESOLVE_RETRY 5
    ...
    resp = at_create_resp(128, 4, 1400 * RT_TICK_PER_SECOND);
    if (resp == RT_NULL)
    {
        LOG_E("no memory for air720 device(%s) response structure.", device->name);
        return -RT_ENOMEM;
