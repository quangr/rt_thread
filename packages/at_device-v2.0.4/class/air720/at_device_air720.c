...
void air720_h_reset(void)
{
    ...
    rt_pin_mode(0x26, PIN_MODE_OUTPUT_OD);
    /* not nead to set pin configuration for m26 device power on */
    rt_pin_write(0x26, PIN_LOW);
    rt_thread_mdelay(2000);
    rt_pin_write(0x26, PIN_HIGH);
...
#define air720_INFO_RESP_TIMO rt_tick_from_millisecond(300)
    ...
    resp = at_create_resp(air720_IEMI_RESP_SIZE, 0, 80 * air720_INFO_RESP_TIMO);
    if (resp == RT_NULL)
    {
...
#define IP_ADDR_SIZE_MAX 16
        ...
        at_resp_set_info(resp, air720_IPADDR_RESP_SIZE, 2, 10 * air720_INFO_RESP_TIMO);
...
#define DNS_ADDR_SIZE_MAX 16
        ...
        at_resp_set_info(resp, air720_DNS_RESP_SIZE, 0, 100 * air720_INFO_RESP_TIMO);
...
#define air720_LINK_DELAY_TIME (10 * RT_TICK_PER_SECOND)
    ...
    while (1)
    {
        ...
        int res = at_obj_exec_cmd(device->client, resp, "AT+CGREG?");
        /* send "AT+CGREG?" commond  to check netweork interface device link status */
        if (res == -2)
        {
            rt_thread_mdelay(air720_LINK_DELAY_TIME * 2);
        ...
        if (res == -1)
        {
            ...
            LOG_E("air720 device(%s) send resboot", device->name);
            continue;
        }
...
#define air720_PING_TIMEO (100 * RT_TICK_PER_SECOND)
...
#define CGATT_RETRY 10
    ...
    while (retry_num--)
    {
        ...
        /* disable echo */
        AT_SEND_CMD(client, resp, 0, 5000, "ATE0");
        /* get module version */
        AT_SEND_CMD(client, resp, 0, 5000, "ATI");
        /* show module version */
        for (i = 0; i < (int)resp->line_counts - 1; i++)
        {
            LOG_I("%s", at_resp_get_line(resp, i + 1));
        }
        AT_SEND_CMD(client, resp, 0, 3000, "AT+CLTS=1");

        /* check SIM card */
        for (i = 0; i < CPIN_RETRY; i++)
        {
            AT_SEND_CMD(client, resp, 2, 20 * RT_TICK_PER_SECOND, "AT+CPIN?");
        ...
        {
            AT_SEND_CMD(client, resp, 0, 10000, "AT+CREG?");
        ...
        {
            AT_SEND_CMD(client, resp, 0, 3000, "AT+CGREG?");
        ...
        {
            AT_SEND_CMD(client, resp, 0, 3000, "AT+CSQ");
        ...
        {
            AT_SEND_CMD(client, resp, 0, 3000, "AT+CGATT?");
        ...
        at_client_obj_send(client, "AT+MIPCLOSE\r\n", 13);
        rt_thread_mdelay(1000);
        //        at_client_obj_send(client,"AT+MIPCLOS\r\n",12);
        //       rt_thread_mdelay(1000);
        /* the device default response timeout is 40 seconds, but it set to 15 seconds is convenient to use. */
        AT_SEND_CMD(client, resp, 2, 10 * 10000, "AT+CIPSHUT");

        /* Set to multiple connections */
        AT_SEND_CMD(client, resp, 0, 3000, "AT+CIPMUX?");
        at_resp_parse_line_args_by_kw(resp, "+CIPMUX:", "+CIPMUX: %d", &qimux);
        if (qimux == 0)
        {
            AT_SEND_CMD(client, resp, 0, 3000, "AT+CIPMUX=1");
        }

        AT_SEND_CMD(client, resp, 0, 3000, "AT+CIPQSEND=1"); //fast send mode ,recommend

        AT_SEND_CMD(client, resp, 0, 9000, "AT+CSTT");
        AT_SEND_CMD(client, resp, 0, 10000 * 1000, "AT+CIICR");

        AT_SEND_CMD(client, resp, 2, 3000, "AT+CIFSR");
...
#if defined(AT_DEVICE_USING_AIR720) || defined(AT_DEVICE_USING_EC200X)
if(mqtt_started==0){

  rt_thread_t tid = rt_thread_create("mqtt start", mqtt_test, (void *)RT_NULL,
                                     4096, 4, 20);
  if (tid != RT_NULL)
  {
    HAL_Printf("MQTT CONNECT!");
    rt_thread_startup(tid);
  }
  mqtt_started=1;
  }
...
static int air720_reboot(struct at_device *device)
{
    rt_pin_mode(0x0b, PIN_MODE_OUTPUT_OD);
    /* not nead to set pin configuration for m26 device power on */
    rt_pin_write(0x0b, PIN_LOW);
    rt_thread_mdelay(2000);
    rt_pin_write(0x0b, PIN_HIGH);
    rt_thread_mdelay(8000);
...
{
    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(10000));
    char parsed_data[10] = {0};
    while (1)
    {
...
MSH_CMD_EXPORT(bug_test, a tcp client sample);